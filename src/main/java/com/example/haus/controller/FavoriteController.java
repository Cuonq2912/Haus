...
public class FavoriteController {
    ...
    public ResponseEntity<ResponseData<FavoriteResponseDto>> addFavorite(
            @AuthenticationPrincipal CustomUserDetails userDetails, @Valid @RequestBody AddFavoriteRequestDto request) {

        String userId = userDetails.getUser().getId();
        var result = favoriteService.addFavorite(userId, request);

        return ResponseUtil.success(HttpStatus.CREATED, SuccessMessage.Product.ADD_FAVORITE_SUCCESS, result);
    }
    ...
    public ResponseEntity<ResponseData<Void>> removeFavorite(@AuthenticationPrincipal CustomUserDetails userDetails,
            @PathVariable @Parameter(description = "ID của sản phẩm cần xóa", example = "123") Long productId) {

        String userId = userDetails.getUser().getId();
        favoriteService.removeFavorite(userId, productId);

        return ResponseUtil.success(HttpStatus.OK, SuccessMessage.Product.REMOVE_FAVORITE_SUCCESS);
    }
    ...
    public ResponseEntity<ResponseData<PaginationResponseDto<ProductResponseDto>>> getFavorites(
        ...
        return ResponseUtil.success(SuccessMessage.Product.GET_FAVORITES_SUCCESS, result);
    }
    ...
    public ResponseEntity<ResponseData<CheckFavoriteResponseDto>> checkFavorite(
        ...
        return ResponseUtil.success(SuccessMessage.Product.CHECK_FAVORITE_SUCCESS, result);
    }
}
