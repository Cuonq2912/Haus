...
public class ProductController {
    ...
    @Operation(summary = "Lấy sản phẩm theo id", description = "Dùng để lấy ra sản phẩm theo id", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<ProductResponseDto>> getProductById(@PathVariable Long id) {
        return ResponseUtil.success(SuccessMessage.Product.GET_PRODUCT_SUCCESS, productService.getProductById(id));
    }

    @Tag(name = "admin-product-controller", description = "Admin Product Management APIs")
    @Operation(summary = "Tạo sản phẩm mới", description = "Dùng để tạo sản phẩm mới", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<ProductResponseDto>> createProduct(
        ...
        return ResponseUtil.success(HttpStatus.CREATED, SuccessMessage.Product.CREATE_PRODUCT_SUCCESS,
                productService.createProduct(request, images));
    }

    @Tag(name = "admin-product-controller", description = "Admin Product Management APIs")
    @Operation(summary = "Cập nhật sản phẩm", description = "Dùng để cập nhật thông tin sản phẩm theo id", security = @SecurityRequirement(name = "Bearer Token"))
    @PutMapping(value = UrlConstant.Product.UPDATE_PRODUCT, consumes = "multipart/form-data")
    public ResponseEntity<ResponseData<ProductResponseDto>> updateProduct(@PathVariable Long id,
            @Valid @RequestPart("request") UpdateProductRequestDto request,
            @RequestPart(value = "images", required = false) MultipartFile[] images) {
        return ResponseUtil.success(SuccessMessage.Product.UPDATE_PRODUCT_SUCCESS,
                productService.updateProduct(id, request, images));
    }

    @Tag(name = "admin-product-controller", description = "Admin Product Management APIs")
    @Operation(summary = "Xóa sản phẩm", description = "Dùng để xóa sản phẩm theo id", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<Void>> deleteProduct(@PathVariable Long id) {
        ...
        return ResponseUtil.success(HttpStatus.OK, SuccessMessage.Product.DELETE_PRODUCT_SUCCESS);
    }

    @Tag(name = "admin-product-controller", description = "Admin Product Management APIs")
    @Operation(summary = "Lấy sản phẩm theo ID category", description = "Dùng để lấy danh sách sản phẩm thuộc category có phân trang")
    ...
    public ResponseEntity<ResponseData<PaginationResponseDto<ProductResponseDto>>> getProductsByCategoryId(
            @PathVariable Long categoryId, @RequestParam(defaultValue = "1") Integer pageNum,
            ...
            @RequestParam(required = false) @Schema(example = "priceRange:under_1m&color:Nâu đậm&material:gỗ&categoryId:1&keyword:a") String search) {
    ...
    @Operation(summary = "Lọc sản phẩm theo nhiều tiêu chí", description = "Lọc sản phẩm theo khoảng giá, màu sắc, chất liệu với phân trang")
    ...
    @Tag(name = "public-product-controller", description = "Public Product APIs")
    @Operation(summary = "Lấy tất cả sản phẩm với phân trang", description = "Lấy danh sách tất cả sản phẩm với phân trang và các tùy chọn sắp xếp productName, createdAt, price")
