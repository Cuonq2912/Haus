...
public class ReviewController {
    ...
    @Operation(summary = "Tạo đánh giá sản phẩm", description = "Cho phép người dùng đánh giá sản phẩm đã mua. Chỉ có thể đánh giá sau khi đơn hàng hoàn thành.", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<ReviewResponseDto>> createReview(
        ...
        return ResponseUtil.success(HttpStatus.CREATED, SuccessMessage.Review.CREATE_REVIEW_SUCCESS,
                reviewService.createReview(userId, productId, request));
    }

    @Operation(summary = "Cập nhật đánh giá", description = "Cho phép người dùng cập nhật đánh giá của mình", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<ReviewResponseDto>> updateReview(
        ...
        String userId = userDetails.getUser().getId();
        return ResponseUtil.success(SuccessMessage.Review.UPDATE_REVIEW_SUCCESS,
                reviewService.updateReview(userId, reviewId, request));
    }

    @Operation(summary = "Xóa đánh giá", description = "Cho phép người dùng xóa đánh giá của mình", security = @SecurityRequirement(name = "Bearer Token"))
    @DeleteMapping(UrlConstant.Review.DELETE_REVIEW)
    public ResponseEntity<ResponseData<Void>> deleteReview(@AuthenticationPrincipal CustomUserDetails userDetails,
            @PathVariable @Parameter(description = "ID của đánh giá cần xóa", example = "1") Long reviewId) {

        String userId = userDetails.getUser().getId();
        reviewService.deleteReview(userId, reviewId);
        return ResponseUtil.success(HttpStatus.OK, SuccessMessage.Review.DELETE_REVIEW_SUCCESS);
    }

    @Operation(summary = "Lấy đánh giá theo ID", description = "Lấy thông tin chi tiết của một đánh giá")
    ...
    public ResponseEntity<ResponseData<ReviewResponseDto>> getReviewById(
        ...
        return ResponseUtil.success(SuccessMessage.Review.GET_REVIEW_SUCCESS, reviewService.getReviewById(reviewId));
    }

    @Tag(name = "public-review-controller", description = "Public Review APIs")
    @Operation(summary = "Lấy danh sách đánh giá của sản phẩm", description = "Lấy tất cả đánh giá của một sản phẩm với phân trang")
    ...
    public ResponseEntity<ResponseData<PaginationResponseDto<ReviewResponseDto>>> getProductReviews(
        ...
        return ResponseUtil.success(SuccessMessage.Review.GET_REVIEWS_SUCCESS,
                reviewService.getProductReviews(productId, paginationRequest));
    }

    @Tag(name = "public-review-controller", description = "Public Review APIs")
    @Operation(summary = "Lấy đánh giá theo số sao", description = "Lọc đánh giá của sản phẩm theo số sao (1-5)")
    ...
    @Tag(name = "public-review-controller", description = "Public Review APIs")
    @Operation(summary = "Lấy thống kê đánh giá sản phẩm", description = "Lấy thống kê số lượng và phần trăm đánh giá theo từng mức sao")
    ...
    public ResponseEntity<ResponseData<RatingStatisticsDto>> getProductRatingStatistics(
        ...
        return ResponseUtil.success(SuccessMessage.Review.GET_RATING_STATISTICS_SUCCESS,
                reviewService.getProductRatingStatistics(productId));
    }

    @Operation(summary = "Lấy danh sách đánh giá của tôi", description = "Lấy tất cả đánh giá mà người dùng đã tạo", security = @SecurityRequirement(name = "Bearer Token"))
    ...
    public ResponseEntity<ResponseData<PaginationResponseDto<ReviewResponseDto>>> getMyReviews(
        ...
        String userId = userDetails.getUser().getId();
        PaginationRequestDto paginationRequest = new PaginationRequestDto(pageNum, pageSize);
        return ResponseUtil.success(SuccessMessage.Review.GET_MY_REVIEWS_SUCCESS,
                reviewService.getMyReviews(userId, paginationRequest));
    }

    @Tag(name = "public-review-controller", description = "Public Review APIs")
    @Operation(summary = "Lấy danh sách đánh giá có số sao cao nhất", description = "Lấy danh sách đánh giá được sắp xếp theo số sao giảm dần. Không phân biệt sản phẩm.")
    ...
    public ResponseEntity<ResponseData<PaginationResponseDto<TopReviewDto>>> getTopReviewsByRating(
        ...
        PaginationRequestDto paginationRequest = new PaginationRequestDto(pageNum, pageSize);
        return ResponseUtil.success(SuccessMessage.Review.GET_TOP_REVIEWS_SUCCESS,
