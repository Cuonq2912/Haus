...
public class PromotionDateValidator implements ConstraintValidator<ValidPromotionDates, PromotionRequestDto> {

    @Override
    public boolean isValid(PromotionRequestDto dto, ConstraintValidatorContext context) {
        if (dto == null)
            return true; // để @NotNull handle riêng
        if (dto.getStartDate() == null || dto.getEndDate() == null)
            return true; // đã có @NotNull riêng
        if (dto.getMinPriceOrder() == null || dto.getMaxPriceOrder() == null)
            return true; // đã có @PositiveOrNull riêng
