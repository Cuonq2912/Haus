...
public interface ProductVariationMapper {
    ...
    @Mapping(target = "discountPercent", expression = "java(productVariation.getProduct()"
            ...
            + "    .orElse(null))")
    @Mapping(target = "daysRemaining", expression = "java(productVariation.getProduct().getCategories().stream()"
            + "    .map(com.example.haus.domain.entity.product.Category::getPromotion)"
            + "    .filter(java.util.Objects::nonNull)"
    ...
    void updateProductVariationFromDto(@MappingTarget ProductVariation productVariation,
    ...
    @Mapping(target = "discountPercent", expression = "java(cartItem.getProductVariation().getProduct()"
            + "     .getCategories().stream()"
            + "    .map(com.example.haus.domain.entity.product.Category::getPromotion)"
            + "    .filter(java.util.Objects::nonNull)"
            + "    .filter(p -> com.example.haus.constant.promotion.PromotionStatus.ACTIVE.equals(p.getStatus()))"
