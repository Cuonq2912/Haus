...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM Review r WHERE r.orderItem.id = :orderItemId AND r.user.id = :userId AND r.deletedAt IS NULL")
    boolean existsByOrderItemIdAndUserId(@Param("orderItemId") Long orderItemId, @Param("userId") String userId);
